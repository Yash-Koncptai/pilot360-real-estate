...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    const fetchUsersData = async () => {
      ...
      try {
        ...
        if (response.data.success) {
          setUsers(response.data.users || []);
          // Ensure properties are set correctly, handle missing or malformed data
          ...
          console.error("Failed to fetch users data:", response.data.message);
          toast.error(`Failed to fetch users data: ${response.data.message}`);
          setProperties([]);
          setUsers([]);
        }
      } catch (err: any) {
  ...
  const handlePropertySave = async (
    ...
    try {
      ...
      payload.append(
        "water_connectivity",
        formData.water_connectivity ? "true" : "false"
      );
      payload.append(
        "electricity_connectivity",
        formData.electricity_connectivity ? "true" : "false"
      );
      payload.append(
        "gas_connectivity",
        formData.gas_connectivity ? "true" : "false"
      );
      payload.append("market_risk", formData.market_risk ? "true" : "false");
      payload.append(
        "regulatory_risk",
        formData.regulatory_risk ? "true" : "false"
      );
      payload.append(
        "financial_risk",
        formData.financial_risk ? "true" : "false"
      );
      payload.append(
        "liquidity_risk",
        formData.liquidity_risk ? "true" : "false"
      );
      payload.append(
        "physical_risk",
        formData.physical_risk ? "true" : "false"
      );
      if (formData.features && formData.features.length) {
        payload.append("features", formData.features.join(","));
      }
